...
in
  stdenv.mkDerivation {
    ...
    buildPhase = ''
      ...
      # Mock some content
      mkdir -p src/content/liederbuch/
      echo -e '---\ntitle: Foo\n---\n# Foo' > src/content/liederbuch/foo.md
      mkdir -p src/content/resos/
      echo -e '---\ntitle: Foo\n---\n# Foo' > src/content/resos/foo.md
      touch src/pages/publikationen/kartenspiel.md
